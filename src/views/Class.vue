<template>
  <navBar />
  <div class="class m-4" style="margin-bottom:250px!important">
...
export default {
  ...
  methods: {
    ...
    postData(arg) {
      ...
      axios
        .post("http://localhost:8085/api/class/create.php", data)
        .then((res) => {
          console.log(res);
        })
        .then(() => {
    ...
    deleteData(id) {
      ...
      let header = {};
      axios
        .post("http://localhost:8085/api/class/delete.php", data, header)
        .then((res) => {
          console.log(res);
        })
        .then(() => {
    ...
    showEditModal(del) {
      ...
      this.getOneE(del);
      window.$("#editClass").hasClass("show")
        ? window.$("#editClass").modal("hide")
        : window.$("#editClass").modal("show");
    },
    showCreateModal(del) {
      if (del) {
        this.intitule = "";
      ...
      window.$("#createClass").hasClass("show")
        ? window.$("#createClass").modal("hide")
        : window.$("#createClass").modal("show");
    },
    updateData(arg) {
      let data;
      if (typeof arg !== "undefined") {
      ...
      axios
        .post("http://localhost:8085/api/class/update.php", data, header)
        .then((res) => {
          console.log(res);
        })
        .then(() => {
