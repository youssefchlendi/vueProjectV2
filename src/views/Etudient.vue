        ...
        <tr v-for="etudient in etudients" :key="etudient.id">
          ...
          <td>
            ...
            <button
              ...
              @click="showEditModal(etudient.id)"
            >
...
export default {
  ...
  methods: {
    getAll() {
      axios
        .get("http://localhost:8085/api/etudient/read.php")
    ...
    postData(arg) {
      ...
      axios
        .post("http://localhost:8085/api/etudient/create.php", data)
        .then((res) => {
          console.log(res);
        })
        .then(() => {
    ...
    showCreateModal() {
      window.$("#createEtudient").hasClass("show")
        ? window.$("#createEtudient").modal("hide")
        : window.$("#createEtudient").modal("show");
    },
    deleteData(id) {
      let data = {
        id: id,
      ...
      let header = {};
      axios
        .post("http://localhost:8085/api/etudient/delete.php", data, header)
        .then((res) => {
          console.log(res);
        })
        .then(() => {
    ...
    showEditModal(id) {
      this.id = id;
      window.$("#editEtudient").hasClass("show")
        ? window.$("#editEtudient").modal("hide")
        : window.$("#editEtudient").modal("show");
    },
    updateData(arg) {
      let data;
      if (typeof arg !== "undefined") {
      ...
      axios
        .post("http://localhost:8085/api/etudient/update.php", data, header)
        .then((res) => {
          console.log(res);
        })
        .then(() => {
    ...
    getOneClass(id) {
      ...
      let tab = {};
      axios
        .get("http://localhost:8085/api/class/read_one.php?id=" + id)
        .then((response) => (oneData = response.data))
        .then(() => {
          Object.keys(oneData).forEach((key) => {
            tab[key] = oneData[key];
